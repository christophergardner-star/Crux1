...
def main():
    ...
    args = {
        ...
        'epochs': 2,
        'lr': 1e-4, # Lion needs lower LR usually
        'log_interval': 50,
        'seed': 1,
        'no_cuda': False,
    }
    ...
    # Initialize Cruxy Optimizer in Meta3 mode with LION features
    print("Initializing Cruxy Stability Engine (Meta-Lion)...")
    optimizer = CruxyOptimizer(
        model.parameters(),
        lr=args['lr'],
        mode="meta3",
        weight_decay=0.01,
        use_lion=True,               # Enable Lion Mode
        use_gc=True,                 # Gradient Centralization
        meta_lr_eta=1e-5,            # Corrected arg name
        meta_interval=10
    )
